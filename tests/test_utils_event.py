...
def test_get_expenses_only_one_expenses():
    ...
    expected_result_category = [
        {'category': 'Супермаркеты', 'amount': 100.0}
    ]
...
def test_get_income_only_one_category():
    ...
    df = pd.DataFrame({
        'Сумма операции': [500.0, -150.0, 500.0],
        'Категория': ['Зарплата', 'Развлечения', 'Зарплата']
    })
    result = get_income(df)
    assert result == {
        'total_amount': 1000.0,
...
def test_get_income_single_operation():
    ...
    assert result == {
        'total_amount': 500.0,
        'main': [{'category': 'Зарплата', 'amount': 500.0}]
    }